        ...
        {magazines?.length > 0 &&
          magazines?.map((magazine) => (
            <Link to={`/magazines/${magazine?._id}`}>
              {" "}
              <div key={magazine?._id} className="relative w-full h-64 rounded-sm overflow-hidden">
                <img src={magazine?.image} alt={magazine?.title} className="w-full h-full object-cover opacity-90" />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent"></div>
                <div className="px-1 text-white flex-col items-center justify-center">
                  <h3 className="text-xl font-semibold absolute bottom-24 px-1">{magazine?.title}</h3>
                  <h3 className=" font-medium absolute bottom-10 px-1">{`${magazine?.description.substring(0, 70)}...`}</h3>
                </div>
              </div>
            </Link>
          ))}
