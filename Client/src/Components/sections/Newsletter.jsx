...
const Newsletter = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/newsletters`, { email });
      ...
      if (response.data.success) {
        ...
        await emailjs.send(import.meta.env.VITE_EMAILJS_SERVICE_ID, import.meta.env.VITE_EMAILJS_TEMPLATE_ID, templateParams, import.meta.env.VITE_EMAILJS_USER_ID);
      ...
      <div className="border border-gray-200 border-r-4 border-b-4 container mx-auto sm:p-10 px-4 sm:py-16 max-sm:py-8 mt-12 flex flex-col md:flex-row items-center justify-between gap-4 ">
        ...
        <div className="flex justify-center items-center">
          <form onSubmit={handleSubmit} className="flex w-full max-w-sm">
            <input
              type="email"
              className="flex-1 border border-gray-200 rounded-l-sm py-2 px-4 shadow-sm focus:ring-2 focus:ring-secondary-black focus:outline-none text-gray-700 placeholder-gray-400 placeholder-shown:font-medium"
            ...
            <button
              type="submit"
              className="bg-neutral-800 text-white px-2 py-3 text-lg rounded-sm hover:bg-neutral-950 transition-all duration-500 font-medium focus:outline-none">
              Subscribe
            </button>
          </form>
        </div>
