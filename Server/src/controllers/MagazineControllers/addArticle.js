...
const addArticle = async (req, res) => {
  try {
    ...
    // Check if user is the creator or a collaborator
    const isAllowed = magazines.some((m) => m.creator.equals(userId)) || magazines.map((m) => m.collaborators.includes(userId));
    ...
    res.status(200).json({ message: "Article added to magazines successfully", magazines });
  } catch (error) {
    res.status(500).json({ message: "Error adding article to magazine", error });
  }
};
